...
- `DB.BeginTx` returns `ErrReadOnly` immediately when `d.readOnly` is true — no transaction is opened. `DB.RunQuery` enforces read-only by checking statement kinds before execution. PRAGMA query_only is NOT used; enforcement is in the API layer.
- `Tx.Rollback()` is idempotent: calling it after `Commit()` returns nil (no-op), following the `database/sql.Tx` convention. This makes `defer tx.Rollback()` safe alongside an explicit `tx.Commit()` call without any special-casing. `Tx.Run()` still returns an error if called after Commit or Rollback.
- Struct-level doc comments on `Tx` are the authoritative lifecycle description; keep them consistent with actual method behaviour to avoid misleading callers.
- `Result.Next` checks `ctx.Err()` at the top (task-014) so context cancellation stops iteration for both sql.Rows and in-memory paths. `Consume` still ignores its context argument (potential follow-up — medium severity finding from skill review).
- `importer.go` import functions use named-return `(retErr error)` with a `defer func() { if retErr != nil { _ = tx.Rollback() } }()` guard — this is the canonical rollback pattern in the codebase. Do not add inline `_ = tx.Rollback()` calls.
