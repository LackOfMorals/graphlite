...
//
// ─────────────────────────────────────────────────────────────────────────────
// Security review (task-033)
// ─────────────────────────────────────────────────────────────────────────────
//
// 1. IMPORT SIZE LIMIT (ErrImportTooLarge)
//    importJSON wraps the caller-supplied io.Reader with a limitedReader that
//    counts bytes through io.LimitReader(r, 500MiB+1). After decoding returns
//    (whether successfully or with an error), limitedReader.exceeded is checked
//    first. If the reader delivered more than 500 MiB the import is aborted with
//    ErrImportTooLarge before any error from the JSON decoder is examined. This
//    prevents JSON-bomb vectors that rely on whitespace inflation to exhaust
//    memory before the parser can detect a structural problem.
//
// 2. JSON NESTING DEPTH LIMIT (ErrImportDepthExceeded)
//    decodeImportJSON first reads the entire document into a json.RawMessage
//    (bounded by the limitedReader above), then calls checkJSONDepth which
//    re-scans the raw bytes with a json.Decoder counting { [ increments and
//    } ] decrements. If depth exceeds importMaxDepth (20) at any point the
//    import is aborted with ErrImportDepthExceeded. This prevents deeply nested
//    JSON ("JSON bomb") payloads from triggering unbounded recursion inside the
//    standard json.Unmarshal decoder.
//
// 3. SQL PARAMETERISATION AUDIT — FINDINGS
//    All user-supplied VALUES (property values, relationship types, label values
//    in WHERE predicates) reach SQLite exclusively via "?" bind parameters.
//    No user-controlled value is ever interpolated directly into a SQL string.
//
//    Property names (e.g. "$.name" in json_extract calls) and SQL table aliases
//    (n0, r1, m0) are interpolated as SQL structural fragments, not bind args.
//    This is safe because:
//      a) Property names originate from the Cypher query string, which is
//         parsed by the cloudprivacylabs/opencypher ANTLR grammar. The grammar
//         restricts property names to valid Cypher identifiers (letters, digits,
//         underscores) — they cannot contain SQL metacharacters or single quotes.
//      b) Table aliases are generated by the aliasCounter in cypher/planner.go
//         (n0, n1, r0, r1, m0, …) — they are never derived from user input.
//      c) The "table" variable in translateSetProp / translateRemoveProp is
//         always either the literal "nodes" or "edges" — never user input.
//    Verdict: no unparameterised user input reaches SQLite query strings.
//
//    Checked files: sql/translator.go, sql/dialect.go, store/sqlite.go,
//    driver.go, session.go.
//
// 4. PATH TRAVERSAL MITIGATION (graphlite.Open)
//    graphlite.Open (driver.go) validates non-":memory:" paths by calling
//    filepath.Clean and checking each component for "..". Any path that
//    contains a ".." component after cleaning is rejected with a descriptive
//    error before the path is forwarded to store.Open / modernc.org/sqlite.
//    The store.Open function itself calls sql.Open("sqlite", uri) which is
//    not path-traversal-aware on its own; the guard in graphlite.Open is the
//    sole mitigation for this class of input.
// ─────────────────────────────────────────────────────────────────────────────
package graphlite
